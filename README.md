...
Las pruebas generan un reporte de cobertura en formato XML mediante la configuracion de `pyproject.toml`:

```toml
addopts = "-v --cov=src --cov-report=term-missing --cov-report=xml"
```

SonarQube Cloud usa este archivo para calcular la cobertura:
